...
from .rgnn import MetaLayer, EdgeModel, NodeModel, MLPGraphIndependent
...
class GalleryMOTMPNet(nn.Module):
    """
    ...
    def __init__(self, model_params, device=torch.device('cpu')):
        """
        ...
        """
        super(GalleryMOTMPNet, self).__init__()

        self.model_params = model_params
        self.device = device

        # Setting up the gallery combinator
        if model_params['gallery_combination'] == 'gru':
            self.combinator = GalleryGRUCombinationLayer(input_size=model_params['combinator_feature_size'],
                                                         hidden_size=model_params['combinator_output_size'],
                                                         num_layers=model_params['combinator_num_gru_layers'])
        else:
            input_size = model_params['combinator_feature_size']*model_params['frames_per_gallery']

            self.combinator = GalleryLinearCombinationLayer(input_size=input_size,
                                                            output_size=model_params['combinator_output_size'])
    ...
    def forward(self, data):
        ...
        node_embeddings = self.combinator(x.to(self.device))
        ...
        latent_edge_feats, latent_node_feats = self.encoder(edge_attr, node_embeddings)
